...
*/
/*
*  问题, 查询关键字时,在新浪云数据库时查询时会触发 RDC 安全机制而报错停止访问.所以这里
*  对三个表的查询分开查询!
*
* */
...
router.get('/list',(req,res)=>
{
    ...
    var sqlJour=`SELECT title,detail,all_author,all_author_mid,pub_year,url,isExh,(SELECT COUNT(id) FROM ${obj.type}) as total FROM ${obj.type} ORDER BY pub_year DESC, id DESC LIMIT ?,?;`;
    var sqlOther = `SELECT title,detail,all_author,all_author_mid,pub_year,isExh,(SELECT COUNT(id) FROM ${obj.type}) as total FROM ${obj.type} ORDER BY pub_year DESC, id DESC LIMIT ?,?;`;
...
router.get('/query',(req,res)=>
{
  ...
  // 采用 async 的方式查询数据库
  (async function(){
    var queryStrJour = ` SELECT title,detail,all_author,all_author_mid,pub_year,type,url,isExh FROM journal_article WHERE ${queryStr} ORDER BY pub_year DESC, id DESC ;`;
    await new Promise(function(open){
      pool.query(queryStrJour,[],(err,results)=>{
        if(err) throw err;
        resultsObj.dataArr = resultsObj.dataArr.concat(results);
        resultsObj.jourCount = results.length;
        open();
      });
    });
    var queryStrConf = ` SELECT title,detail,all_author,all_author_mid,pub_year,type,url,isExh FROM conf_article WHERE ${queryStr} ORDER BY pub_year DESC, id DESC ;`;
    await new Promise(function(open){
      pool.query(queryStrConf,[],(err,results)=>{
        if(err) throw err;
        resultsObj.dataArr = resultsObj.dataArr.concat(results);
        resultsObj.confCount = results.length;
        open();
      });
    });
    var queryStrPatent = ` SELECT title,detail,all_author,all_author_mid,pub_year,type,url,isExh FROM patent WHERE ${queryStr}  ORDER BY pub_year DESC, id DESC ;`;
    await new Promise(function(open){
      pool.query(queryStrPatent,[],(err,results)=>{
        if(err) throw err;
        resultsObj.dataArr = resultsObj.dataArr.concat(results);
        resultsObj.patentCount = results.length;
        open();
      });
    });
    var htmlArr = addData(resultsObj,kwsArr);
...
function addData(data,kwsArr){
  ...
  {
    n = 1;
    var jourSum = data.jourCount;
    var confSum = data.confCount;
    var patentSum = data.patentCount;
    data = data.dataArr;
    if(jourSum || confSum || patentSum)
      var info = `
    ...
    `;
      ...
      info = `<p>抱歉,未查询到与<span>${kwsArr}</span>相关的记录!</p><p>请核对您输入关键字是否正确.<a href="javascript:history.go(0)">确定</a></p>`;
  }
  else
    info = `<span>共 ${data[0].total} 篇文章记录！</span>`;
...
function preDeal(prev,obj,index,arr){
  ...
  if(obj.isExh == 1) // 确认展示
  {
    ...
    var nameString = '';
    // 处理人名链接 <p> /之内的部分/  </p>
    var regName = /[a-z\s]+/i;  // Yiping Wang*
